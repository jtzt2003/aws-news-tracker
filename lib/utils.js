...
async function generateSummary(title, content) {
  ...
  if (!anthropic || !process.env.ANTHROPIC_API_KEY) {
    console.log('[TIMING] No API key, using truncation');
    // Fallback to first 200 characters if no API key
    return content.substring(0, 200) + '...';
  }

  try {
    console.log(`[TIMING] Calling Claude API for: "${title.substring(0, 50)}..."`);
...
Summary:`
    ...
    console.log(`[TIMING] Claude API responded in ${Date.now() - summaryStart}ms`);
    return message.content[0].text.trim();
  } catch (error) {
    console.error(`[ERROR] Failed generating summary in ${Date.now() - summaryStart}ms:`, error.message);
    return content.substring(0, 200) + '...';
  }
}
