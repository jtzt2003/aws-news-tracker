...
async function fetchAllAnnouncements() {
  ...
  // Filter to last 7 days first (before processing)
  ...
  console.log(`Filtered to ${recentAnnouncements.length} announcements from last 7 days (from ${allRawAnnouncements.length} total)`);

  // Process announcements (categorize + summarize)
  // Limit to 100 to avoid too many API calls
  const processed = [];
  const BATCH_SIZE = 5; // Process 5 at a time
  const DELAY_MS = 500; // 0.5 second delay (faster to avoid timeout)
  const MAX_ITEMS = 20; // Reduced from 100 to prevent 30-second timeout
  ...
  for (let i = 0; i < Math.min(recentAnnouncements.length, MAX_ITEMS); i += BATCH_SIZE) {
    ...
    batchResults.forEach((result, idx) => {
      if (result.status === 'fulfilled') {
        ...
        console.error(`Error processing announcement ${i + idx}:`, result.reason?.message);
      }
    });
  ...
  console.log(`Successfully processed ${processed.length} out of ${Math.min(recentAnnouncements.length, MAX_ITEMS)} announcements`);
  ...
  console.log(`Fetched and processed ${uniqueAnnouncements.length} announcements`);
